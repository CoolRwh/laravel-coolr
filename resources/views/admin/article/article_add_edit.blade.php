                                    ...
                                    <div class="ibox-content">
                                        <div class="form-group">
                                            ...
                                            <div class="mdui-textfield mdui-textfield-floating-label">
                                                ...
                                                <textarea class="mdui-textfield-input" name="title" maxlength="50"> @if(isset($article->id)) {{$article->title}} @endif </textarea>
                                            </div>
                                        ...
                                        <div class="form-group">
                                            <div>
                                                ...
                                                <select class="mdui-select" mdui-select="{position: 'bottom'}"
                                                    ...
                                                    @foreach($cates as $k)
                                                        <option value="{{$k->id}}"  @if($article->cate_id == $k->id) selected @endif  >{{$k->cate_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div>
                                                <div class="form-group">
                                                    <input id="file-5" class="file" type="file" multiple
                                                           ...
                                                           data-upload-url="{{route('admin.image.update')}}">
                                                    <input type="hidden" name="titlepic"
                                                           value="http://log.coolr.top/static/web/images/tu.jpg">
                                                </div>
...
<script type="text/javascript" charset="utf-8"
        src="{{@asset('static/admin/plugins/ueditor/ueditor.config.js')}}"></script>
<script type="text/javascript" charset="utf-8"
        src="{{@asset('static/admin/plugins/ueditor/ueditor.all.min.js')}}"></script>
<!--建议手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
<!--这里加载的语言文件会覆盖你在配置项目里添加的语言类型，比如你在配置项目里配置的是英文，这里加载的中文，那最后就是中文-->
<script type="text/javascript" charset="utf-8"
        src="{{@asset('static/admin/plugins/ueditor/lang/zh-cn/zh-cn.js')}}"></script>
